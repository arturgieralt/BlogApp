import { ARTICLES_FETCH_REQUEST, fetchArticlesSuccess, ARTICLE_ADD_REQUEST, addArticleSuccess } from "../actions/articles";
...
export const fetchArticlesEpic = action$ => action$.pipe(
  ...
  export const addArticleEpic = action$ => action$.pipe(
    ofType(ARTICLE_ADD_REQUEST),
    mergeMap(action =>
      ajax.post('https://localhost:3001/articles', {...action.payload}).pipe(

...
import TokenFactory, { Authorization } from '../../factories/Token/TokenFactory';
...
export default class AuthorizeMiddleware {
    public constructor(private RoleService: IRoleService, private TokenService: ITokenService) {}

    public authorize = (roles: string[] = [], scopes: string[] = TokenFactory.Permissions[Authorization]) => [
        ...
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            try {
                const tokenStatus: ITokenModel = await this.TokenService.getSingle(user.tokenId);
